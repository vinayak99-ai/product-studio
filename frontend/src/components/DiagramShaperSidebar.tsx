...
import { extractFile, openDiagramShaperGenerateSocket } from '../lib/api'
import type { DiagramShaperWsProgressMessage, GenerateDiagramShaperResponse } from '../diagramShaperTypes'

type Status = 'idle' | 'calling_llm' | 'done' | 'error'

interface DiagramShaperSidebarProps {
  onResult: (response: GenerateDiagramShaperResponse) => void
}

const STATUS_LABEL: Record<Status, string> = {
  idle: '',
  calling_llm: 'Placing shapes…',
  done: 'Ready.',
  error: 'Something went wrong.',
}

export function DiagramShaperSidebar({ onResult }: DiagramShaperSidebarProps) {
  ...
  const handleGenerate = useCallback(() => {
    ...
    const onMessage = (message: DiagramShaperWsProgressMessage) => {
    ...
    closeSocketRef.current = openDiagramShaperGenerateSocket(material, prompt, onMessage, onError)
    ...
    <aside className="flex h-full w-[360px] shrink-0 flex-col gap-4 overflow-y-auto border-r border-neutral-200 bg-white p-4">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="e.g. Turn this into a diagram. We'll pick the type -- process, decision flow, hierarchy, architecture, or timeline -- that fits best."
