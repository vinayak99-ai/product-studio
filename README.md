...
| **Diagram Slides** | Ready | Source material + a prompt → a hand-composed diagram -- linear process, decision flow, hierarchy, architecture, timeline, swimlane, or process (shape encodes type, one reserved accent, no layout-engine routing) -- rendered server-side and exported full-bleed into a PPTX slide. |
